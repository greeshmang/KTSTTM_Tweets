...
public class BTM {
            ...
            for (String doc; (doc = br.readLine()) != null;) {
                ...
                for (String word : words) {
                    ...
                    if (word2IdVocabulary.containsKey(word)) {
                        ...
                        // System.out.println("word is there in word2Idvocabulary, so is kept in document with the index found which is "+word2IdVocabulary.get(word));

                    }
                    else {
                         // System.out.println("word is not there in word2Idvocabulary"+word2IdVocabulary.get(word));

                        indexWord += 1;
                      //  System.out.println("increment indexword"+indexWord);

                        word2IdVocabulary.put(word, indexWord);
                       // System.out.println("put the word in  word2IdVocabulary"+word);
            ...
            throws IOException
    {
                 ...
                 //   System.out.println("word2 "+word2);
                    if(word1<word2){
                        ...
                        if(!oneCop.containsKey(itmeNum)){
                            oneCop.put(itmeNum,0);// check whether it is already there, if not add it
                        }
                        oneCop.put(itmeNum,oneCop.get(itmeNum)+1); // increment the number if it is already there
       ...
       // for(int bitermIndex=0;bitermIndex<this.biterms.size();bitermIndex++){
           // System.out.print("bitermIndex= "+bitermIndex);
           // System.out.println(", topicId="+this.topic_of_biterms[bitermIndex]);
      //  }
...
System.out.println("writeTopTopicalWords()");
        BufferedWriter writer = new BufferedWriter(new FileWriter(folderPath
                + expName + ".topWords"));
    ...
    public void writeTopicWordPros()
            throws IOException
    {
        System.out.println("writeTopicWordPros");
        BufferedWriter writer = new BufferedWriter(new FileWriter(folderPath
                + expName + ".phi"));
    ...
    public void writeDocTopicPros()
            throws IOException
    { System.out.println("writeDocTopicPros()");
        BufferedWriter writer = new BufferedWriter(new FileWriter(folderPath
                + expName + ".theta"));
    ...
    public static void main(String args[])
            throws Exception
    {
        BTM btm = new BTM("/home/cec/STTM/src/dataset/tweetnews.txt",3, 0.1, 0.1, 1000, 10, "tweet");
        btm.inference();
    }
}
