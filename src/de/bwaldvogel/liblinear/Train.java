...
public class Train {
    ...
    private double do_cross_validation() throws IOException {
        ...
        if (param.solverType.isSupportVectorRegression()) {
            ...
            int total_correct = 0;
            BufferedWriter writer = new BufferedWriter(new FileWriter("Inferedlabel.txt"));
            

            for (int i = 0; i < prob.l; i++)
            { String label=String.valueOf(target[i]);
                writer.write(label);
                System.out.print((i+1)+" infered label="+target[i]);
                System.out.print("correct label="+ prob.y[i]);
    ...
    public double run(String[] args) throws IOException, InvalidInputDataException {
        ...
        System.out.println("file identified as "+inputFilename);
